...
// Replace using a simple string replacement for each language block
transJs = transJs.replace(
    'navContactMobile: "Let\\'s Talk",',
    'navContactMobile: "Let\\'s Talk",\n' + enAdditions
);

transJs = transJs.replace(
    'navContactMobile: "تواصل معنا",',
    'navContactMobile: "تواصل معنا",\n' + arAdditions
);

// For fr and nl, they both have 'navContactMobile: "Contact",'. We can replace the first occurrence for fr, and the second for nl.
let frIndex = transJs.indexOf('navContactMobile: "Contact",');
transJs = transJs.substring(0, frIndex) + 'navContactMobile: "Contact",\n' + frAdditions + transJs.substring(frIndex + 'navContactMobile: "Contact",'.length);

let nlIndex = transJs.indexOf('navContactMobile: "Contact",', frIndex + frAdditions.length);
transJs = transJs.substring(0, nlIndex) + 'navContactMobile: "Contact",\n' + nlAdditions + transJs.substring(nlIndex + 'navContactMobile: "Contact",'.length);
